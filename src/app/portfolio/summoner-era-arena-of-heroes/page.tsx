...
const ACCENT = "#10b981";
const ACCENT_SOFT = "#facc15";

function FactIcon({ name }: { name: string }) {
  const common = "h-5 w-5 text-[#10b981]";
  if (name === "calendar") {
    return (
      <svg
        viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      <span className="block">
        <span style={{ color: ACCENT_SOFT }}>Arena</span>{" "}
        <span className="text-white/85">of</span>{" "}
        <span style={{ color: ACCENT }}>Heroes</span>
      </span>
      <span className="mt-1 block text-[0.62em] font-black tracking-[0.06em] md:text-[0.58em]">
        <span className="text-white/72">Animation</span>
        <span className="text-white/40"> — </span>
        <span style={{ color: ACCENT }}>Summoner Era</span>
        <span className="text-white/40"> · </span>
        <span className="text-white/55">cinematic hero pack</span>
      </span>
    </h1>
  );
}
...
function PanelWorkflow() {
  const steps = [
    { n: "01", t: "Concept", sub: "Storyboard" },
    { n: "02", t: "Spine rig", sub: "Skinning" },
  ...
  return (
    <div className="mt-4 flex h-full flex-col rounded-2xl border border-white/10 bg-white/2 p-5">
      ...
      <div className="relative mt-5 flex-1">
        <div
          className="pointer-events-none absolute left-[12%] right-[12%] top-[20px] h-px bg-linear-to-r from-emerald-400/55 via-yellow-400/35 to-white/10"
          aria-hidden
        />
        <div className="relative grid grid-cols-4 gap-1">
          {steps.map((s, i) => (
            <div key={s.n} className="flex flex-col items-center text-center">
              <div className="relative flex h-10 w-10 items-center justify-center rounded-full border border-emerald-400/55 bg-[#08130d] text-[12px] font-black text-emerald-300 shadow-[0_0_0_3px_rgba(8,18,12,1)]">
                {s.n}
                {i < steps.length - 1 && (
                  <svg
                    viewBox="0 0 24 24"
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="absolute right-[-10px] top-1/2 h-3 w-3 -translate-y-1/2 text-emerald-400/70"
                    aria-hidden
                  >
                    <path d="M9 6l6 6-6 6" />
                  </svg>
...
function SectionTitle({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="mb-8">
      <div className="text-[11px] font-bold uppercase tracking-[0.34em] text-emerald-400">
        {eyebrow}
      </div>
      <h2
...
function RelatedCard({
  ...
  const className =
    "group overflow-hidden rounded-[24px] border border-white/10 bg-[#131313] transition-colors hover:border-emerald-400/40";
  const body = (
    <>
  ...
  return (
    <>
      ...
      <main className="bg-[#080808] text-white">
        <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.20),transparent_42%),radial-gradient(circle_at_top_right,rgba(250,204,21,0.16),transparent_42%),radial-gradient(ellipse_at_70%_0%,rgba(56,189,248,0.10),transparent_45%),linear-gradient(180deg,#07120d_0%,#050807_75%)] pt-20 md:pt-24">
          <div
          ...
          >
            <div className="flex flex-col">
              ...
              <div className="mt-4 text-[11px] font-bold uppercase tracking-[0.34em] text-emerald-400">
                {projectMeta.eyebrow}
              </div>
              ...
              <div className="mt-2 text-[12px] text-white/55">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="font-semibold text-yellow-200 underline-offset-2 hover:underline"
                >
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-emerald-400 px-5 py-2.5 text-[11px] font-black uppercase tracking-[0.2em] text-black transition-colors hover:bg-emerald-300"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-5 py-2.5 text-[11px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:border-emerald-400/50 hover:bg-emerald-400/10"
                >
            ...
            <div className="flex h-full flex-col">
              <div className="relative aspect-video w-full overflow-hidden rounded-[26px] border border-white/10 bg-[#0a1611] shadow-[0_28px_70px_-28px_rgba(16,185,129,0.34),0_32px_85px_-32px_rgba(250,204,21,0.24)]">
                <Image
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/55 via-black/5 to-[#0a1611]/40"
                  aria-hidden
                />
                <div
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="mt-3 flex flex-1 flex-wrap content-around gap-2.5">
                    {projectMeta.fields.map((field) => (
                      <span
                        key={field}
                        className="h-fit rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1.5 text-[11px] font-semibold text-emerald-100"
                      >
                        {field}
                      </span>
                    ))}
        ...
        <section className="border-b border-white/8 bg-[#0a1611] py-14 md:py-18">
          <div
          ...
          >
            ...
            <div className="overflow-hidden rounded-[28px] bg-[#0e1d17]">
              {showcaseModules.map((module) => {
                if (module.variant === "duo") {
                  return (
                    <div
                      key={module.id}
                      className="grid grid-cols-1 gap-px bg-white/5 md:grid-cols-2"
                    >
                      {module.srcs.map((src, idx) => (
                        ...
                        <img
                          key={`${module.id}-${idx}`}
                          src={src}
                          alt={`${projectMeta.title} module ${module.id} part ${idx + 1}`}
