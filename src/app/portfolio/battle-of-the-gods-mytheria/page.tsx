...
function FactIcon({ name }: { name: string }) {
  const common = "h-5 w-5 text-[#f5b84a]";
  if (name === "calendar") {
    return (
      <svg
        viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      <span className="block">
        <span style={{ color: ACCENT }}>Battle</span>
        <span className="text-white/55"> of the </span>
        <span style={{ color: ACCENT_SOFT }}>Gods</span>
      </span>
      <span className="mt-1 block text-[0.62em] font-black tracking-[0.06em] md:text-[0.58em]">
        <span className="text-white/72">Mytheria</span>
        <span className="text-white/40"> · </span>
        <span style={{ color: ACCENT }}>Login Screen</span>
      </span>
    </h1>
  );
}
  ...
  return (
    <div className="mt-4 flex h-full flex-col rounded-2xl border border-white/10 bg-white/2 p-5">
      ...
      <div className="relative mt-5 flex-1">
        <div
          className="pointer-events-none absolute left-[12%] right-[12%] top-[20px] h-px bg-linear-to-r from-amber-400/55 via-violet-400/35 to-white/10"
          aria-hidden
        />
        <div className="relative grid grid-cols-4 gap-1">
          {steps.map((s, i) => (
            <div key={s.n} className="flex flex-col items-center text-center">
              <div className="relative flex h-10 w-10 items-center justify-center rounded-full border border-amber-300/55 bg-[#141414] text-[12px] font-black text-amber-200 shadow-[0_0_0_3px_rgba(20,20,20,1)]">
                {s.n}
                {i < steps.length - 1 && (
                  <svg
                    viewBox="0 0 24 24"
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="absolute right-[-10px] top-1/2 h-3 w-3 -translate-y-1/2 text-amber-300/70"
                    aria-hidden
                  >
                    <path d="M9 6l6 6-6 6" />
                  </svg>
...
function SectionTitle({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="mb-8">
      <div className="text-[11px] font-bold uppercase tracking-[0.34em] text-amber-300">
        {eyebrow}
      </div>
      <h2
...
function RelatedCard({
  ...
  const className =
    "group overflow-hidden rounded-[24px] border border-white/10 bg-[#131313] transition-colors hover:border-amber-300/40";
  const body = (
    <>
...
export default function BattleOfTheGodsDetailPage() {
  return (
    <>
      <SiteHeader />
      <main className="bg-[#080808] text-white">
        <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(245,184,74,0.18),transparent_40%),radial-gradient(circle_at_top_right,rgba(167,139,250,0.18),transparent_42%),radial-gradient(ellipse_at_70%_0%,rgba(124,58,237,0.12),transparent_45%),linear-gradient(180deg,#13101e_0%,#070510_75%)] pt-20 md:pt-24">
          <div
          ...
          >
            <div className="flex flex-col">
              ...
              <div className="mt-4 text-[11px] font-bold uppercase tracking-[0.34em] text-amber-300">
                {projectMeta.eyebrow}
              </div>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-amber-300 px-5 py-2.5 text-[11px] font-black uppercase tracking-[0.2em] text-black transition-colors hover:bg-amber-200"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-5 py-2.5 text-[11px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:border-amber-300/50 hover:bg-amber-300/10"
                >
            ...
            <div className="flex h-full flex-col">
              <div className="relative aspect-video w-full overflow-hidden rounded-[26px] border border-white/10 bg-[#0f0a1c] shadow-[0_28px_70px_-28px_rgba(245,184,74,0.30),0_32px_85px_-32px_rgba(167,139,250,0.26)]">
                <Image
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/55 via-black/5 to-[#0f0a1c]/40"
                  aria-hidden
                />
                <div
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="mt-3 flex flex-1 flex-wrap content-around gap-2.5">
                    {projectMeta.fields.map((field) => (
                      <span
                        key={field}
                        className="h-fit rounded-full border border-amber-300/25 bg-amber-300/10 px-3 py-1.5 text-[11px] font-semibold text-amber-100"
                      >
                        {field}
                      </span>
                    ))}
        ...
        <section className="border-b border-white/8 bg-[#191919] py-14 md:py-18">
          ...
          >
            ...
            <div className="overflow-hidden rounded-[28px] bg-[#0f0a1c]">
              {showcaseModules.map((module) => {
                ...
                if (module.variant === "duo") {
                  return (
                    <div
                      key={module.id}
                      className="grid grid-cols-1 gap-2 bg-[#0f0a1c] p-2 sm:grid-cols-2 sm:gap-3 sm:p-3"
                    >
                      {module.srcs.map((src, i) => (
                        ...
                        <img
                          key={`${module.id}-${i}`}
                          src={src}
                          alt={`${projectMeta.title} module ${module.id} ${i + 1}`}
