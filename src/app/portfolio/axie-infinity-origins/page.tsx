  ...
  return (
    ...
    >
      <span className="block">
        <span className="text-[#ffb000]">Axie Infinity</span>{" "}
        <span className="text-white">Origins</span>
      </span>
      <span className="mt-1 block text-[0.62em] font-black tracking-[0.06em] md:text-[0.58em]">
        <span className="text-[#ff8c3a]">Animation</span>
        <span className="text-white/40"> · </span>
        <span className="text-white/72">Spine 2D</span>
      </span>
    </h1>
  );
}
...
function RelatedCard({
  ...
  const className =
    "group overflow-hidden rounded-[24px] border border-white/10 bg-[#131313] transition-colors hover:border-[#ff8c3a]/40";
  const body = (
    <>
...
export default function AxieInfinityOriginsDetailPage() {
  return (
    <>
      <SiteHeader />
      <main className="bg-[#080808] text-white">
        <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.12),transparent_40%),radial-gradient(circle_at_top_right,rgba(255,140,58,0.15),transparent_42%),radial-gradient(ellipse_at_70%_0%,rgba(255,176,0,0.08),transparent_45%),linear-gradient(180deg,#141414_0%,#0a0a0a_75%)] pt-20 md:pt-24">
          <div
          ...
          >
            ...
            <div className="flex h-full flex-col">
              <div className="relative aspect-video w-full overflow-hidden rounded-[26px] border border-white/10 bg-[#0b1218] shadow-[0_28px_70px_-28px_rgba(56,189,248,0.22),0_32px_85px_-32px_rgba(255,140,58,0.32)]">
                <Image
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/50 via-black/5 to-[#0b1218]/35"
                  aria-hidden
                />
                <div
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="mt-3 flex flex-1 flex-wrap content-around gap-2.5">
                    {projectMeta.fields.map((field) => (
                      <span
                        key={field}
                        className="h-fit rounded-full border border-[#ff8c3a]/25 bg-[#ff8c3a]/10 px-3 py-1.5 text-[11px] font-semibold text-[#ffd4b0]"
                      >
                        {field}
                      </span>
                    ))}
            ...
            <div className="overflow-hidden rounded-[28px] bg-[#dbc5a5]">
              {showcaseModules.map((module) => {
                if (module.variant === "vimeo") {
                  return (
                    <div key={module.id} className="bg-[#dbc5a5]">
                      <div
                      ...
                      >
                        <iframe
                          src={module.embedSrc}
                          title={`${projectMeta.title} — ${module.id}`}
                ...
                if (module.variant === "trio") {
                  return (
                    <div
                      key={module.id}
                      className="grid grid-cols-1 bg-[#dbc5a5] px-3 py-6 sm:grid-cols-3 sm:px-4 md:py-10"
                    >
