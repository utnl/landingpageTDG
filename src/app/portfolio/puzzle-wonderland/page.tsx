  ...
  return (
    ...
    >
      <span className="block">
        <span style={{ color: ACCENT_SOFT }}>Puzzle</span>{" "}
        <span style={{ color: ACCENT }}>Wonderland</span>
      </span>
      <span className="mt-1 block text-[0.62em] font-black tracking-[0.06em] md:text-[0.58em]">
        <span className="text-white/72">Character</span>
        <span className="text-white/40"> · </span>
        <span style={{ color: ACCENT }}>animation</span>
        <span className="text-white/40"> — </span>
        <span className="text-white/55">cute mobile puzzle</span>
      </span>
    </h1>
  );
}
  ...
  return (
    <div className="mt-4 flex h-full flex-col rounded-2xl border border-white/10 bg-white/2 p-5">
      ...
      <div className="relative mt-5 flex-1">
        <div
          className="pointer-events-none absolute left-[12%] right-[12%] top-[20px] h-px bg-linear-to-r from-cyan-400/55 via-amber-400/35 to-white/10"
          aria-hidden
        />
        <div className="relative grid grid-cols-4 gap-1">
          {steps.map((s, i) => (
            <div key={s.n} className="flex flex-col items-center text-center">
              <div className="relative flex h-10 w-10 items-center justify-center rounded-full border border-cyan-400/55 bg-[#06151a] text-[12px] font-black text-cyan-300 shadow-[0_0_0_3px_rgba(6,21,26,1)]">
                {s.n}
                {i < steps.length - 1 && (
                  <svg
                    viewBox="0 0 24 24"
...
function ShowcaseSectionLabel({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-center bg-[#0a1418] px-6 py-8 md:py-12">
      <div className="flex items-center gap-4">
        ...
        <h3
          ...
          style={{ color: ACCENT, fontFamily: "var(--font-rajdhani)" }}
        >
          {label}
        </h3>
        <span
...
export default function PuzzleWonderlandDetailPage() {
  return (
    <>
      <SiteHeader />
      <main className="bg-[#080808] text-white">
        <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.20),transparent_42%),radial-gradient(circle_at_top_right,rgba(251,191,36,0.16),transparent_42%),radial-gradient(ellipse_at_70%_0%,rgba(244,114,182,0.10),transparent_45%),linear-gradient(180deg,#06141a_0%,#050708_75%)] pt-20 md:pt-24">
          <div
          ...
          >
            ...
            <div className="flex h-full flex-col">
              <div className="relative aspect-video w-full overflow-hidden rounded-[26px] border border-white/10 bg-[#08161c] shadow-[0_28px_70px_-28px_rgba(34,211,238,0.34),0_32px_85px_-32px_rgba(251,191,36,0.24)]">
                <Image
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/55 via-black/5 to-[#08161c]/40"
                  aria-hidden
                />
                <div
        ...
        <section className="border-b border-white/8 bg-[#08161c] py-14 md:py-18">
          <div
          ...
          >
            ...
            <div className="overflow-hidden rounded-[28px] bg-[#0b1d24]">
              {showcaseModules.map((module) => {
                ...
                if (module.variant === "videoEmbed") {
                  return (
                    <div
                      key={module.id}
                      className="flex justify-center bg-[#0a1418] px-4 py-6 md:py-10"
                    >
                      <div
                      ...
                      >
                        <iframe
                          src={module.src}
                          title={`${projectMeta.title} login screen reel`}
                          loading="lazy"
                ...
                if (module.variant === "square") {
                  return (
                    <div
                      key={module.id}
                      className="flex justify-center bg-[#0a1418] px-4 py-3"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={module.src}
                        alt={`${projectMeta.title} module ${module.id}`}
                        className="block h-auto w-full max-w-[680px] rounded-lg"
                        loading="lazy"
                      />
                    </div>
                  );
